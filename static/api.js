...
window.SXHR = {
	post:function(url,query, data, callback) {
		query&&(url+='?'+SXHR.formQuery(query));
	...
	request:function(method,url,body,callback){
		...
		(body===null||body===undefined)?xhr.send():xhr.send(body);
		return xhr;
	},
	formQuery:function(query){
		...
		if (query&&(typeof query ==='object')) {
			var querys = []; 
			for (var i in query) {
				if (query[i]&&(typeof query[i]=== 'object')) query[i] = JSON.stringify(query[i]); 
				querys.push((query[i]!==undefined)?(encodeURIComponent(i) + '=' + encodeURIComponent(query[i])):i); 
			}
			tQuery += querys.join('&'); 
		}
		return tQuery;
...
window.SAPI={
	get:function(api, query, callback){
		...
		SXHR.get(SAPI.siteRoot+'api/',Object.assign({api:api},query),function(err,xhr){
	...
	post:function(api,query, body, callback){
		var hasCallback=typeof callback==='function',okFunc,noFunc;
		SXHR.post(SAPI.siteRoot+'api/',Object.assign({api:api},query),body,function(err,xhr){
			SAPI.parseResult(err,xhr,hasCallback?callback:function(err,result,code,xhr){
				if(err)noFunc(err);
				else{
					okFunc(result);
